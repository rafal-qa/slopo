...
allowed-tools: Bash(slopo *)
argument-hint: "[cluster hash] by default next"
arguments: [cluster]
...
!`slopo agent-analyze --config-version=1 --single --cluster=$cluster`

# Report structure

- Similar code report contains one cluster with file paths and line ranges.
